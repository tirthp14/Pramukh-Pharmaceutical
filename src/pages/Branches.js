...
function Branches() {
    ...
    const handleMouseOver = (event, locationDot) => {
        const locationName = locationDot.getAttribute("data-location");
        const locationEmail = locationDot.getAttribute("data-email");
        const locationHours = locationDot.getAttribute("data-hours");
        const locationAddress = locationDot.getAttribute("data-address");
        const locationPhone = locationDot.getAttribute("data-phone");
        const locationMap = locationDot.getAttribute("data-map");
    ...
    return (
        <div>
            ...
            <section className="bg-stone-100 flex flex-col-reverse">
                <div id="tooltip" className="font-contactUs tracking-wider font-semibold md:absolute flex items-center justify-center w-auto h-auto m-5 md:m-0 p-3 md:p-5 text-sm bg-mainBlue shadow-md rounded-2xl pointer-events-none text-white md:opacity-0 z-50" style={tooltipStyle} dangerouslySetInnerHTML={{ __html: tooltipContent }}></div>
                ...
                <section className="pb-7 md:pb-24">
                    <div className="mx-auto p-6 pt-2 md:pt-6 md:w-[70%]">
                        ...
                        <p className="lg:pb-6 font-light text-center sm:text-xl text-zinc-800">
                            Got a prescription to fill? Need health advice? Or want to explore our range of healthcare products? Visit any of our six branches conveniently located across the city. Our dedicated team is ready to assist you at every location. Find the nearest pharmacy and experience top-notch service.
                        </p>
                    </div>
